    ...
    <div className="pt-28 pb-24 px-4 md:px-[3.5cm]">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-x-[1.5cm] gap-y-[1cm]">
        {hotelProperties.map((property) => {
          ...
          return (
            <div key={property.id}>
              <Link href={`/hotels/${property.id}`} className="group relative block w-full aspect-[3/4.5] overflow-hidden bg-ink/5">
                <Image
                  src={cover.imageUrl}
                  alt={property.name}
                  fill
                ...
                <div className="absolute inset-0 flex items-end justify-center" style={{ paddingBottom: 'calc(1.5rem + 2cm)' }}>
                  <span className="text-background font-display font-light text-4xl text-center px-4 drop-shadow-lg">
                    {property.name}
                  </span>
                </div>
              </Link>
